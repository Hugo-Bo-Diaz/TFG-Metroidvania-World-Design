...
void UIhealthbar::Render()
{
	...
	Engine->GetModule<::Render>().RenderTexture(TextureHB, x, y, begin, 30, RenderQueue::RENDER_UI);
	//middle sections
	int offset = size_in_image;
	for (int i = 0; i < max_res; ++i)
	{
		Engine->GetModule<::Render>().RenderTexture(TextureHB, x+offset, y, middle, 30, RenderQueue::RENDER_UI);
		offset += size_in_image;
	}

	//final sections
	Engine->GetModule<::Render>().RenderTexture(TextureHB, x+offset, y, end, 30, RenderQueue::RENDER_UI);

	//bars
	offset = size_in_image;
	for (int i = 0; i < curr_res; ++i)
	{
		Engine->GetModule<::Render>().RenderTexture(TextureHB, x + offset, y, bar, 25, RenderQueue::RENDER_UI);
		offset += size_in_image;
	}
	...
	if (res == MANA)//if it is mana it should a last partial section
	{
		...
		Engine->GetModule<::Render>().RenderTexture(TextureHB, x + offset, y, last_bar, 25, RenderQueue::RENDER_UI);
	}
}
...
void UIhealthbar::RenderDebug()
{
	...
	Engine->GetModule<::Render>().RenderRect({ (int)x,(int)y,lTotalW,begin.h }, RXColor{ 255, 255, 255, 255 }, false, RenderQueue::RENDER_DEBUG, -10, true);
