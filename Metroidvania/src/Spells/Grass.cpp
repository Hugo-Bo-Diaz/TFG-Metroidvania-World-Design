...
void Grass::Loop(float dt)
{
	...
	//grasshook------------------------------------------------------------------------------------------------------------------
	if (Engine->GetModule<Input>().GetInput(BUTTON_3) == KEY_DOWN)
	{
		if (!hooked && !hook_out)
		{
			float x,y;
...
void Grass::Render()
{
	if (hooked || hook_out)
	{
		Engine->GetModule<::Render>().RenderTexture(spells, hook_position_x - hook.w / 2, hook_position_y - hook.h / 2, hook, -2, RenderQueue::RENDER_GAME, 0);
