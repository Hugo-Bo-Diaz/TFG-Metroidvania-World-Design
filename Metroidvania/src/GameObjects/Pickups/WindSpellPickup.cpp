...
bool WindSpellPickup::Loop(float dt)
{
	if(Engine->GetModule<ProgressTracker>().GetBaseSaveSection()->GetChild("SpellsUnlock")->GetValue("Wind"))
	{
	...
	for (std::vector<collision>::iterator it = collisions.begin(); it != collisions.end(); it++)
	{
		if ((*it).object != this)
		{
			...
			{
				((Player*)((*it).object))->unlock_spell(WIND);
				Engine->GetModule<ProgressTracker>().GetBaseSaveSection()->GetChild("SpellsUnlock")->SetValue("Wind", 1);
				Engine->GetModule<SceneController>().DeleteObject(this);
				UItextbox* textbox= new UItextbox("","congratulations you unlocked wind!",TextBoxColor::RED, 15, 4, 272, 420, 2, 0.2);
...
bool WindSpellPickup::Render()
{
	Engine->GetModule<::Render>().RenderTexture(spell_books, collider.x, collider.y, windspellbook, 10);
	return true;
}
