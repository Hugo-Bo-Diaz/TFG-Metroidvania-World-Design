...
void UISelectMenu::Loop()
{
	...
	case SELECT_MENU_LORE:
	{
		...
		if (abs(amount_of_movement) > 0.6 && !stop_inputs && App->trk->active_logs.size() > 1)
		{
			if (amount_of_movement > 0 && current_lore < App->trk->active_logs.size()-1)
			{
				current_lore += 1;
...
void UISelectMenu::Render()
{
	...
	case SELECT_MENU_LORE:
	{
		...
		if (App->trk->active_logs.size() > 0)
		{
			for (int i = cam_y_coord; i < MAX_LORE_CAPACITY_MENU+cam_y_coord && i<App->trk->active_logs.size(); ++i)
			{
				...
				std::string temp = App->trk->active_logs[i]->title;
			...
			description->ChangeText(App->trk->active_logs[current_lore]->text.c_str());
