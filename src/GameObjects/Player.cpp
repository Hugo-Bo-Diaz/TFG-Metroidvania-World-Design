...
bool Player::Loop(float dt)
{
	...
	*/
	...
	if (Engine->GetModule<Input>().GetInput(BUTTON_1) == KEY_DOWN && (grounded))
	{
		jump = true;
	}
	else
	...
	{
		if (Engine->GetModule<Input>().GetInput(L_SHOULDER) == KEY_DOWN)
		{
			cycle_spell(-1);
		}
		if (Engine->GetModule<Input>().GetInput(R_SHOULDER) == KEY_DOWN)//START
		{
			cycle_spell(1);
		}
	}
...
{
	if (!Engine->GetModule<Debug>().IsDebugActive())
	{
...
void Player::Destroy()
{
	...
	delete spells[GRASS];
	Engine->GetModule<Camera>().FollowObject(nullptr);
